...
const QueueList: TransformListFunctionType = function(
    ...
    return new Promise(async (resolve, reject) => {
        const { QueueUrls } = await AWSClient.listQueues()
        .promise();
        const queues = await Promise.all(
            QueueUrls.map(q => AWSClient.getQueueAttributes({ QueueUrl: q, AttributeNames: ['All'] }).promise()));
    
        const resources = queues.map((q, i) => service.Queue(q.Attributes.QueueArn, {
