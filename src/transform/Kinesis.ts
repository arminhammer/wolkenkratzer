...
const StreamList: TransformListFunctionType = function(
  ...
  return new Promise(async (resolve, reject) => {
    const { StreamNames } = await AWSClient.listStreams().promise();
    const streams = await Promise.all(
      StreamNames.map(s =>
        AWSClient.describeStreamSummary({ StreamName: s }).promise()
      )
    );
    const tags = await Promise.all(
      StreamNames.map(s =>
        AWSClient.listTagsForStream({ StreamName: s }).promise()
      )
    );

    const resources = streams.map((s, i) =>
      service.Stream(s.StreamDescriptionSummary.StreamName, {
