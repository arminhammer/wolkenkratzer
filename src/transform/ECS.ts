...
const ClusterList: TransformListFunctionType = function(
  ...
  return new Promise(async (resolve, reject) => {
    const { clusterArns } = await AWSClient.listClusters().promise();
    const clusterNames = clusterArns.map(c => c.split(':')[5].slice(8));
    const resources = clusterNames.map(x =>
      service.Cluster(x, {
        ClusterName: x,
      })
    );
...
const ServiceList: TransformListFunctionType = function(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const serviceList = await Promise.all(
      clusterArns.map(c => AWSClient.listServices({ cluster: c }).promise())
    );
    const services = flatten(
      await Promise.all(
        serviceList.map((s, i) =>
          AWSClient.describeServices({
            cluster: clusterArns[i],
            services: s.serviceArns,
          }).promise()
        )
      )
    );
    const describedServices = flatten(
      services.map(x => x.services.map(y => y))
    );
    const resources = describedServices.map(x =>
      service.Service(x, {
...
const TaskDefinitionList: TransformListFunctionType = function(
  ...
  return new Promise(async (resolve, reject) => {
    const {
      taskDefinitionArns,
    } = await AWSClient.listTaskDefinitions().promise();
    const taskDefinitionList = await Promise.all(
      taskDefinitionArns.map(x =>
        AWSClient.describeTaskDefinition({ taskDefinition: x }).promise()
      )
    );
    const resources = taskDefinitionList.map(x =>
      service.TaskDefinition(x, {
