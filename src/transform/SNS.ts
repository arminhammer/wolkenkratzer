...
const TopicList: TransformListFunctionType = function(
    ...
    return new Promise(async (resolve, reject) => {
        const { Topics } = await AWSClient.listTopics()
        .promise();

        const topics = await Promise.all(
            Topics.map(x => AWSClient.getTopicAttributes({ TopicArn: x.TopicArn }).promise()));
        const subscriptions = await Promise.all(
            Topics.map(x => AWSClient.listSubscriptionsByTopic({ TopicArn: x.TopicArn }).promise()));
    
        const resources = topics.map((x, i) => service.Topic(x.Attributes.DisplayName, {
...
const TopicPolicyList: TransformListFunctionType = function(
    ...
    return new Promise(async (resolve, reject) => {
        ...
        const resources = topics.map((x, i) => service.TopicPolicy(x.Attributes.TopicArn, {
...
const SubscriptionList: TransformListFunctionType = function(
    ...
    return new Promise(async (resolve, reject) => {
        const { Subscriptions } = await AWSClient.listSubscriptions()
        .promise();
    
        const resources = Subscriptions.map((x, i) => service.Subscription(x.SubscriptionArn.split(':')[6], {
            Endpoint : x.Endpoint,
            Protocol : x.Protocol,
            TopicArn : x.TopicArn
          }));
