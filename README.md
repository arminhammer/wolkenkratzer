...
Wolkenkratzer is a Javascript library that programmatically generates AWS CloudFormation templates.

The library targets 100% feature parity with CloudFormation. This is accomplished by scraping the public documentation and using that to build the data model. The scraper and json data model are in the [cfn-doc-json-stubs](https://github.com/arminhammer/cfn-doc-json-stubs) project. Markdown documentation for the data model is available at [doc.md](https://github.com/arminhammer/cfn-doc-json-stubs/blob/master/doc.md).
...
The following script ([test.js](https://github.com/arminhammer/wolkenkratzer/blob/master/examples/test.js)) adds a VPNGateway to the template:
...
When the script is run with ```javascript node examples/test.js```, this results in the following CloudFormation template:
...
[YAML output](https://github.com/arminhammer/wolkenkratzer/blob/master/examples/test.js) is also supported:
...
An example of creating a resource with one argument (ec21), and another (ec22) with a JSON object:
  ...
  "Resources": {
    ...
    "ec22": {
      ...
      "Properties": {
        ...
        "SecurityGroups": [
          {
            "GroupName": "awseb-e-qyaht32vin-stack-AWSEBSecurityGroup-xxxxx",
            "GroupId": "sg-abaxxxx"
          }
        ],
        "SourceDestCheck": true,
        "SubnetId": "subnet-bcxxxxxx"
      }
    }
  },
  "AWSTemplateFormatVersion": "2010-09-09"
...
#### Resource support
Wolkenkratzer supports all CloudFormation resources. This is made possible by scripts that scrape the CloudFormation documentation pages and generate the resource code files. The scraper and json data model can be found in the [cfn-doc-json-stubs](https://github.com/arminhammer/cfn-doc-json-stubs) project.

Documentation for all available supported resources and attribute types can be found [here](https://github.com/arminhammer/cfn-doc-json-stubs/blob/master/doc.md).
...
####  Ref
```javascript
bucket.BucketName = new wk.Intrinsic.Ref(bucketName);
```
or (shorter version):
```javascript
bucket.BucketName = new wk.Ref(bucketName);
```
...
####  FnSub
```javascript
LambdaInvokePermission.SourceArn = new wk.Intrinsic.Sub('arn:aws:s3:::${BucketName}');
```
or (shorter version):
```javascript
...
####  FnFindInMap
```javascript
distConfigOrigin.DomainName.join('', [
  new Ref(s3BucketForWebsiteContent),
  new wk.Intrinsic.FnFindInMap(
...
Parameters take two parameters: the first parameter is the logical id of the Parameter, and the other is the object that represents the attributes of the Parameter:
