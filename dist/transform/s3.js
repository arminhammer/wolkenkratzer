...
const s3Service = (0, _service.Service)('S3');
...
function S3BucketTransform(bucketName, logicalName, awsObj) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const bucket = {};
    return s3Client.getBucketVersioning({ Bucket: bucketName }).promise().then(versionData => {
    ...
    .then(corsData => {
      ...
      return s3Client.getBucketLifecycleConfiguration({ Bucket: bucketName }).promise();
    }).catch(e => {
      ...
      return s3Client.getBucketNotification({ Bucket: bucketName }).promise();
    }).then(notificationData => {
      if (Object.keys(notificationData).length > 0) {
        bucket.NotificationConfiguration = notificationData;
      }
      return s3Client.getBucketReplication({ Bucket: bucketName }).promise();
    }).then(replData => {
